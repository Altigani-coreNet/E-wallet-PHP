...
const AccountVerification = ({ formData, setFormData, onNextStep }) => {
    ...
    const sendVerificationCode = async () => {
        ...
        try {
            if (verificationStep === 'email') {
                const response = await fetch(`${AUTH_SERVICE_BASE_URL}/api/softpos/register/send-verification-code`, {
                    method: 'POST',
                    headers: {
    ...
    const handleVerificationComplete = async (code) => {
        ...
        try {
            if (verificationStep === 'email') {
                const response = await fetch(`${AUTH_SERVICE_BASE_URL}/api/softpos/register/verify-code`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                ...
                <div className="text-center mb-10">
                    ...
                    <div className="alert alert-success mb-5">
                        <div className="d-flex flex-column">
                            ...
                            {/* <span className="fs-7 mt-2">A welcome email has been sent to {userData?.email} with instructions to complete your merchant registration.</span> */}
                ...
                <div className="text-center">
                    <button
                        ...
                        onClick={async () => {
                            ...
                            try {
                                const response = await fetch(`${AUTH_SERVICE_BASE_URL}/api/softpos/register/user`, {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
