...
const CompanyProfile = ({ formData, setFormData, fieldErrors }) => {
    ...
    const fetchCountries = async (searchTerm = '') => {
        ...
        try {
            const url = searchTerm 
                ? `${AUTH_SERVICE_BASE_URL}/api/softpos/countries/select?search=${encodeURIComponent(searchTerm)}`
                : `${AUTH_SERVICE_BASE_URL}/api/softpos/countries/select`;
            ...
            if (data.status) {
                ...
                console.log('Countries loaded:', data.data.length); // Debug log
            }
        } catch (error) {
    ...
    const handleCountrySelect = (country) => {
        ...
        // Update formData using the prop function from parent
        setFormData('country', country.id);
        setFormData('city', ''); // Reset city when country changes
    ...
    const fetchCities = async (countryId) => {
        ...
        try {
            const response = await fetch(`${AUTH_SERVICE_BASE_URL}/api/softpos/cities/select?country_id=${countryId}`);
            const data = await response.json();
            console.log('Cities API response:', data); // Debug log
            if (data.status) {
                setCities(data.data);
                setFilteredCities(data.data);
                console.log('Cities loaded:', data.data.length); // Debug log
            }
        } catch (error) {
    ...
    const fetchBusinessTypes = async () => {
        ...
        try {
            const response = await fetch(`${AUTH_SERVICE_BASE_URL}/api/softpos/business-types/select`);
            const data = await response.json();
            if (data.status) {
    ...
    const fetchTermsContent = async () => {
        ...
        try {
            const response = await fetch(`${AUTH_SERVICE_BASE_URL}/api/softpos/contract-terms`);
            const data = await response.json();
    ...
    useEffect(() => {
        fetchCountries(); // This will show loading state
        fetchBusinessTypes();
    }, []);

    // Load cities when country changes
    useEffect(() => {
        console.log('formData.country changed:', formData.country); // Debug log
        console.log('selectedCountry:', selectedCountry); // Debug log
        ...
        if (formData.country) {
            ...
            setSelectedCity(null);
            setCitySearchTerm('');
            // Clear city in formData when country changes
            setFormData('city', '');
        } else {
            setCities([]);
            setFilteredCities([]);
    ...
    return (
        <div className="w-100">
            ...
            <div className="row">
                ...
                {/* Location Information Section - Moved to the end */}
                <div className="col-12">
