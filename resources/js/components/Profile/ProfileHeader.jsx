...
const resolveAuthAssetUrl = (path) => {
    ...
    const normalizedBase = AUTH_SERVICE_BASE_URL.replace(/\/+$/, '');
...
const ProfileHeader = ({ user, merchant, profileCompletion, merchantCompletion, activeTab, onTabChange }) => {
    ...
    const logoSource = user?.profile_image || merchant?.logo || merchant?.logo_url;
    const logoUrl = resolveAuthAssetUrl(logoSource);
    const displayInitial = logoError || !logoUrl;
    const fallbackSource = (merchant?.business_name || merchant?.name || user?.name || 'NA').trim();
                    ...
                    <div className="me-7 mb-4">
                        <div className="symbol symbol-100px symbol-lg-160px symbol-fixed position-relative">
                            {!displayInitial && logoUrl ? (
                                <img
                                    src={logoUrl}
                                    alt={merchant?.business_name || user?.name || 'Merchant Logo'}
                                    className="rounded"
