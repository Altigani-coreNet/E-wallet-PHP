...
const TerminalGroupEditForm = ({ initialData }) => {
    console.log('🏗️ TerminalGroupEditForm component initializing...', { initialData });
    
    // Detect if we're in merchant context
    const isMerchantContext = window.location.pathname.includes('merchant');
    console.log('🔍 Context detected:', isMerchantContext ? 'Merchant' : 'Admin');
    
    // Get admin token from the root element (retrieved from session in blade template)
    const getAdminToken = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        terminal_ids: initialData.terminals ? initialData.terminals.map(t => t.id) : [],
        user_group_ids: initialData.user_groups ? initialData.user_groups.map(ug => ug.id) : [],
    ...
    const [terminalGroup, setTerminalGroup] = useState(initialData);
    ...
    useEffect(() => {
        ...
        // Set Admin API token from session
        const adminToken = getAdminToken();
        if (adminToken) {
            axios.defaults.headers.common['Authorization'] = `Bearer ${adminToken}`;
            console.log('✅ Admin API token set successfully');
        } else {
            console.warn('⚠️ Admin API token not found');
        }
    }, []);
    ...
    const loadParentGroups = async () => {
        console.log('🔄 Loading parent groups...');
        try {
            ...
            console.log('✅ Parent groups loaded successfully:', response.data.length, 'groups found');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isMerchantContext) {
                ...
                const response = await axios.put(`/merchant/terminal-groups/${initialData.id}`, formData);
                
                if (response.data && response.data.success) {
                    // Success - redirect to merchant terminal groups index
                    ...
                    if (response.data && response.data.message) {
                        ...
                        setErrors({ general: 'An error occurred while updating the terminal group.' });
                    }
                }
            } else {
                ...
                const response = await fetch(`/admin/terminal-groups/${initialData.id}`, {
                    method: 'PUT',
                    headers: headers,
                ...
                if (result.success) {
                    ...
                    setErrors({ general: result.message || 'An error occurred while updating the terminal group.' });
                }
            }
        } catch (error) {
    ...
    console.log('🎨 TerminalGroupEditForm component rendering...');
...
export default TerminalGroupEditForm; 