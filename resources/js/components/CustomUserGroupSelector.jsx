...
const CustomUserGroupSelector = ({ 
    ...
    useEffect(() => {
        if (searchTerm) {
            ...
            // If no search, load all user groups
            loadUserGroups();
        }
    ...
    const loadUserGroups = async () => {
        ...
        try {
            ...
            const response = await axios.get('/api/softpos/admin/user-groups/select', config);
    ...
    const fetchFilteredUserGroups = async () => {
        ...
        try {
            ...
            const response = await axios.get(`/api/softpos/admin/user-groups/select?${params.toString()}`, config);
    ...
    const handleSelectAll = () => {
        if (selectedUserGroups.length === filteredUserGroups.length) {
            ...
            // If not all are selected, select all filtered user groups
    ...
    return (
        <div className={`custom-user-group-selector ${className}`}>
            ...
            {/* User Groups Header - Similar to Brands & Models Header */}
                        ...
                        <div className="d-flex align-items-center gap-3">
                            ...
                            {filteredUserGroups.length > 0 && (
                                <div className="d-flex align-items-center gap-2">
                                    <button
                                        className={`btn btn-sm ${selectedUserGroups.length === filteredUserGroups.length ? 'btn-success' : 'btn-outline-primary'}`}
                                        onClick={() => handleSelectAll()}
                                        title={selectedUserGroups.length === filteredUserGroups.length ? 'Deselect All' : 'Select All'}
                                    >
                                        <i className={`ki-duotone ${selectedUserGroups.length === filteredUserGroups.length ? 'ki-check' : 'ki-plus'} fs-5 me-1`}>
                                            <span className="path1"></span>
                                            <span className="path2"></span>
                                        </i>
                                        {selectedUserGroups.length === filteredUserGroups.length ? 'Deselect All' : 'Select All'}
                                    </button>
