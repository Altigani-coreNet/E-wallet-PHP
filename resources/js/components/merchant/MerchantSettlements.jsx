...
const MerchantSettlements = () => {
    ...
    const fetchSettlements = useCallback(async (page = 1, size = perPage) => {
        setLoading(true);
        try {
            const token = getApiToken();
            const baseUrl = getApiBaseUrl();
            const response = await axios.get(`${baseUrl}/settlements/data`, {
                params: {
    ...
    const fetchStatistics = useCallback(async () => {
        try {
            const token = getApiToken();
            const baseUrl = getApiBaseUrl();
            const response = await axios.get(`${baseUrl}/settlements/statistics`, {
                params: {
                    merchant_id: merchantId
                },
                headers: {
    ...
    useEffect(() => {
        fetchSettlements(currentPage, perPage);
        fetchStatistics();
    }, [fetchSettlements, fetchStatistics, currentPage, perPage]);
                    ...
                    <div className="d-flex flex-wrap flex-stack mb-6">
                        ...
                        <div className="d-flex align-items-center gap-2 my-2">
                            ...
                            <button
                                ...
                                onClick={() => fetchSettlements(currentPage, perPage)}
                                disabled={loading}
                            >
                                        ...
                                        {loading ? (
                                            ...
                                            settlements.map((settlement) => (
                                                <tr key={settlement.id}>
                                                    ...
                                                    <td>{settlement.batch_number}</td>
                                                    <td>
                                                        <span className={`badge badge-light-${getStatusColor(settlement.status)}`}>
                                                            {settlement.status.charAt(0).toUpperCase() + settlement.status.slice(1)}
                                                        </span>
                                                    </td>
                                                    <td>{settlement.currency_code} {parseFloat(settlement.total_amount).toFixed(2)}</td>
