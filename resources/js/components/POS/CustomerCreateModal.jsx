...
import { API_V_1 } from '../../utils/constants';
...
const CustomerCreateModal = ({ isOpen, onClose, onCustomerCreated }) => {
    ...
    // Initialize Select2 when customer groups are loaded
    useEffect(() => {
        if (customerGroups.length > 0 && select2Ref.current && window.$) {
            // Destroy existing Select2 if it exists
            if (window.$(select2Ref.current).hasClass('select2-hidden-accessible')) {
                window.$(select2Ref.current).select2('destroy');
            }
            
            // Initialize Select2
            window.$(select2Ref.current).select2({
    ...
    useEffect(() => {
        return () => {
            if (select2Ref.current && window.$ && window.$(select2Ref.current).hasClass('select2-hidden-accessible')) {
                window.$(select2Ref.current).select2('destroy');
            }
        };
    }, []);
    ...
    // Handle Select2 change for customer group
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await apiPost(`${API_V_1}/customer/store`, formData);
            ...
            if (response.success) {
                ...
                if (response.error?.errors) {
                    ...
                    setErrors({ general: response.error?.message || 'Failed to create customer' });
                }
            }
        } catch (error) {
                        ...
                        <div className="modal-body">
                            ...
                            <div className="col-md-6 mb-3">
                                    ...
                                    <select 
                                        ...
                                        className={`form-select select2 ${errors.customer_group_id ? 'is-invalid' : ''}`}
