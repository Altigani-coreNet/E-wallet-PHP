...
const UserGroupEditForm = () => {
    // Get initial data from DOM data attributes
    const rootElement = document.getElementById('user-group-edit-form-root');
    const initialUserGroup = JSON.parse(rootElement.dataset.userGroup || '{}');
    const initialUsers = JSON.parse(rootElement.dataset.users || '[]');
    const initialMerchant = JSON.parse(rootElement.dataset.merchant || '{}');
    const initialBranch = JSON.parse(rootElement.dataset.branch || '{}');

    const [formData, setFormData] = useState({
        merchant_id: initialUserGroup.merchant_id || '',
        branch_id: initialUserGroup.branch_id || '',
        name: initialUserGroup.name || '',
        description: initialUserGroup.description || '',
        user_ids: initialUsers.map(u => u.id) || []
    });
    ...
    useEffect(() => {
        const loadMerchants = async () => {
            try {
                ...
                // If we have an initial merchant, add it to options if not present
                if (initialMerchant.id && !merchants.find(m => m.id === initialMerchant.id)) {
                    setMerchantOptions(prev => [...prev, {
                        id: initialMerchant.id,
                        text: initialMerchant.name,
    ...
    useEffect(() => {
        ...
        const loadRelated = async () => {
            ...
            try {
                ...
                // If we have an initial branch, add it to options if not present
                if (initialBranch.id && !branchesArray.find(b => b.id === initialBranch.id)) {
                    setBranchOptions(prev => [...prev, {
                        id: initialBranch.id,
                        text: initialBranch.name,
    ...
    const handleSubmit = async (e) => {
        ...
        setErrors({});
        try {
            const res = await fetch(`/admin/user-groups/${initialUserGroup.id}`, {
                method: 'PUT',
                headers: {
            ...
            if (!res.ok) {
                ...
                else setErrors({ general: result?.message || 'Failed to update user group.' });
                return;
            }
