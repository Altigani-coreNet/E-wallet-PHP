...
const MerchantUserGroupEditForm = ({ userGroup, merchantId }) => {
    const [formData, setFormData] = useState({
        merchant_id: merchantId || '',
        branch_id: userGroup?.branch_id || '',
        name: userGroup?.name || '',
        description: userGroup?.description || '',
        user_ids: userGroup?.users?.map(user => user.id.toString()) || []
    });
    ...
    // Update formData when userGroup prop changes
    useEffect(() => {
        if (userGroup) {
            setFormData({
                merchant_id: merchantId || '',
    ...
    const handleSubmit = async (e) => {
        ...
        setErrors({});
        try {
            const res = await fetch(`/merchant/user-groups/${userGroup.id}`, {
                method: 'PUT',
                headers: {
            ...
            if (!res.ok) {
                ...
                if (res.status === 422) {
                    ...
                    // Other errors
                    setErrors({ general: result?.message || `Error ${res.status}: Failed to update user group.` });
                }
                return;
            }
...
export default MerchantUserGroupEditForm;
