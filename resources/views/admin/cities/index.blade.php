...
@section('breadcrumbs')
    ...
    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1" >
        ...
        <!--end::Item-->
        <!--begin::Item-->
        <li class="breadcrumb-item text-muted">{{ __('translation.cities') }}</li>
        <li class="breadcrumb-item">
            <span class="bullet bg-gray-500 w-5px h-2px"></span>
        </li>
        <li class="breadcrumb-item text-muted">{{ __('translation.cities_list') }}</li>
        <!--end::Item-->
    </ul>
    <!--end::Breadcrumb-->
@endsection
...
$(document).ready(function() {
    var table = $('#cities_table').DataTable({
        ...
        drawCallback: function (settings) {
            ...
            $('[data-kt-check-target="#cities_table .form-check-input"]').prop('checked', false);
            $('#record-ids').val('');
    ...
    $(document).on('change', '.status-toggle', function() {
        var toggle = $(this);
        var cityId = toggle.data('id');
        var isChecked = toggle.is(':checked');
        
        $.ajax({
            url: '{{ route("admin.cities.show", ":id") }}'.replace(':id', cityId),
            method: 'GET',
            data: {
                status: true
            },
    ...
    $(document).on('change', '[data-kt-check-target="#cities_table .form-check-input"]', function() {
