...
<div style="min-width: 100px">
    ...
    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
        <div class="menu-item px-3">
            <a href="{{ route('merchant.terminals.show', ['terminal' => $id]) }}" class="menu-link px-3">
                {{ __('translation.view') }}
            </a>
        </div>
        <div class="menu-item px-3">
            <a href="{{ route('merchant.terminals.show', ['status' => true, 'terminal' => $id]) }}" class="menu-link px-3">
                {{ __('translation.change_status') }}
            </a>
        </div>
        <div class="menu-item px-3">
            <a href="{{ route('merchant.terminals.edit', $id) }}" class="menu-link px-3">
                {{ __('translation.edit') }}
            </a>
        </div>
        <div class="menu-item px-3">
            <form action="{{ route('merchant.terminals.destroy', $id) }}" method="post">
                @csrf
                @method('delete')
