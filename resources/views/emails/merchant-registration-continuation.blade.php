...
<head>
    ...
    <title>{{ __('emails.merchant_continuation_subject') }}</title>
    <style>
        body {
...
<body>
    <div class="header">
        <h1>{{ __('emails.merchant_continuation_heading') }}</h1>
        <p>{{ __('emails.merchant_continuation_subheading') }}</p>
    </div>

    <div class="content">
        <h2>{{ __('emails.merchant_continuation_greeting', ['name' => $fullName]) }}</h2>

        <p>{!! __('emails.merchant_continuation_intro', ['business' => '<strong>'.e($businessName).'</strong>']) !!}</p>
