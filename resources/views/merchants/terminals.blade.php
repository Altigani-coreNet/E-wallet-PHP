...
@section('main-head', __('translation.merchant_terminals'))
@section('page_title', __('translation.merchant_terminals'))
@section('breadcrumbs')
    <!--begin::Breadcrumb-->
    ...
    <div class="row gy-5 g-xl-8">
        <!--begin::Col-->
            ...
            {{-- <div class="card-body py-3"> --}}
                @if(isset($merchant))
                    <x:merchant-profile-header :merchant="$merchant" active-tab="terminals" />
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="card mb-5 mb-xl-10" id="kt_merchant_terminals">
                                <!--begin::Card header-->
                                <div class="card-header cursor-pointer">
                                    <!--begin::Card title-->
                                    <div class="card-title m-0">
                                        <h3 class="fw-bolder m-0">{{ __('translation.merchant_terminals') }}</h3>
                                    </div>
                                    <div class="d-flex align-items-center">
                                        ...
                                        <a href="{{ route('terminals.create') }}?merchant_id={{ $merchant->id }}" class="btn btn-primary align-self-center">
                                ...
                                <div class="card-body p-9">
                                    ...
                                    <!--begin::Terminals Table-->
                                    <div class="table-responsive">
                                        <table class="table align-middle table-row-dashed fs-6 gy-5" id="merchant-terminals-table">
                                            <!--begin::Table head-->
                                            <thead>
                                            <!--begin::Table row-->
                                                ...
                                                <th class="w-10px pe-2">
                                                    <div class="form-check form-check-sm form-check-custom form-check-solid me-3">
                                                        <input class="form-check-input" type="checkbox" data-kt-check="true"
                                                               data-kt-check-target="#merchant-terminals-table .form-check-input" value="1"/>
                                                    </div>
                                                </th>
                                                <th class="text-dark">{{ __('translation.id') }}</th>
                                                <th class="min-w-125px text-dark">{{ __('translation.terminal_name') }}</th>
                                                <th class="min-w-125px text-dark">{{ __('translation.terminal_id') }}</th>
                                                <th class="text-dark">{{ __('translation.device_id') }}</th>
                                                <th class="text-dark">{{ __('translation.branch') }}</th>
                                                <th class="text-dark">{{ __('translation.terminal_group') }}</th>
...
@push('scripts')
    <script>
        ...
        let merchantTerminalsTable = $('#merchant-terminals-table').DataTable({
            dom: "tiplr"
            , serverSide: true
            , processing: true
            , autoWidth: false
        ...
        $(window).on('resize', function () {
            $('#merchant-terminals-table').css('width', '100%');
            $('.table-responsive').css('width', '100%');
        });

        $("[name='search']").attr("placeholder", "{{__('translation.terminal_name,_terminal_id,_device_id')}}");

        $("[name='search']").on('keyup', function () {
            search = $(this).val();
            console.log(search);
            merchantTerminalsTable.ajax.reload();
        });
