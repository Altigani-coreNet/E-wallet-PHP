...
class ReportController extends Controller
{
    ...
    public function profitLossExport(ReportFilterRequest $request)
    {
        try {
            $startDate = $request->input('start_date') ?? date('Y-01-01');
            $endDate = $request->input('end_date') ?? date('Y-m-d');

            return $this->reportService->exportProfitAndLoss($startDate, $endDate);
        } catch (\Throwable $e) {
            Log::error('ReportController@profitLossExport: '.$e->getMessage());

            return $this->ErrorMessage('Failed to export profit and loss report', null, 500);
        }
    }
