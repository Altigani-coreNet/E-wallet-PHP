...
class CustomerOtpService
{
    ...
    public function findVerifiedSmsOtp(string $token, string $phone): \App\Models\CustomerOtp
    {
        ...
        if (!$otp) {
            throw new \InvalidArgumentException('Phone not verified or OTP expired');
        }
    ...
    private function sendSms(string $phone, int $code): string
    {
        Log::info("[STUB] Sending SMS OTP to {$phone}: {$code}");
