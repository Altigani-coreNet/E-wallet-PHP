...
class TransactionService
{
    ...
    public function getAuditTrail($transactionId): Collection
    {
        Transaction::findOrFail($transactionId);

        return TransactionLog::getAuditTrail($transactionId);
    }

    public function getTransactionHistory($transactionId, $limit = null): Collection
    {
        Transaction::findOrFail($transactionId);

        return TransactionLog::getTransactionHistory($transactionId, $limit);
    }
    ...
    public function processPartialRefund(
        ...
        if ($transaction->status !== 'approved') {
            throw new \InvalidArgumentException(
                'Transaction must be in approved status to be refunded. Current status: ' . $transaction->status
            );
        }

        $transaction->refund($amount, $description, $metadata);

        return $transaction->fresh();
    }

    public function getTransactionSummary(?string $merchantId = null, ?string $dateFrom = null, ?string $dateTo = null): array
    {
        $merchantId = $merchantId ?? Auth::guard('external')->user()?->merchant?->id;

        $query = Transaction::query()->when($merchantId, fn ($q) => $q->where('merchant_id', $merchantId));
    ...
    public function getTransactionsWithRefunds(?string $merchantId = null, int $limit = 50): Collection
    {
        $merchantId = $merchantId ?? Auth::guard('external')->user()?->merchant?->id;
    ...
    public function getVoidedTransactions(?string $merchantId = null, int $limit = 50): Collection
    {
        $merchantId = $merchantId ?? Auth::guard('external')->user()?->merchant?->id;
    ...
    public function getCancelledTransactions(?string $merchantId = null, int $limit = 50): Collection
    {
        $merchantId = $merchantId ?? Auth::guard('external')->user()?->merchant?->id;
