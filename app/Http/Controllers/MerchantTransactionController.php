...
class MerchantTransactionController extends Controller
{
    ...
    public function refundTransaction(Request $request, Transaction $transaction)
    {
        ...
        $request->validate([
            'amount' => 'required|numeric|min:0.01|max:' . ($transaction->refundable_amount ?? $transaction->amount),
            'reason' => 'required|string|max:255'
        ]);
        ...
        try {
            ...
            $refundTransaction = Transaction::create([
                'transaction_id' => 'REF_' . $transaction->transaction_id . '_' . time(),
            ...
            // Update original transaction refundable amount
            $transaction->update([
                'refundable_amount' => ($transaction->refundable_amount ?? $transaction->amount) - $request->amount
            ]);
