...
class PublicPlanController extends Controller
{
    ...
    /**
     * Resolve a currency symbol string for API responses (Spatie translatable-safe).
     */
    private function resolveCurrencySymbol(?Currency $currency): ?string
    {
        ...
        $symbol = $currency->getTranslation('symbol', $locale, false)
            ?: $currency->getTranslation('symbol', 'en', false)
            ?: $currency->symbol;

        return LocaleString::one($symbol);
    }
        ...
        try {
            ...
            $plans = Plan::with([
                ...
                ->map(function ($plan) use ($detectedCountryId) {
                    ...
                    $planData['currency_symbol'] = $this->resolveCurrencySymbol($bestPrice['currency']);
